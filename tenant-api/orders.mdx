...
`additionalBaggage` is optional. Each `optionId` must come from the latest offer response. `quantity` must be between `1` and the option's `maximumQuantity`.

The response returns normalized baggage selections, and `totalCents` includes the flight and all selected additional baggage.
